...
public class Client implements Runnable {
	...
	public void setPlayersCoords(String str) {
		...
		for (int i = 0; i < players.length; i++) {
			...
			arena.setPlayersCoords(name_and_coord[0], Double.parseDouble(x_and_y[0]), Double.parseDouble(x_and_y[1]));
		}
	}
	...
	public void setPlayersVcoords(String str) {
		...
		for (int i = 0; i < players.length; i++) {
			...
			arena.setPlayersVcoords(name_and_vcoord[0], Double.parseDouble(vcoord[0]), Double.parseDouble(vcoord[1]), Double.parseDouble(vcoord[2]), Double.parseDouble(vcoord[3]), Double.parseDouble(vcoord[4]));
		}
	}
	...
	public void setObstaclesCoords(String str) {
		...
		for (int i = 0; i < obstacles.length; i++) {
			...
			if (id_and_coord.length == 0) {
				...
				arena.setObstaclesCoords(i, Double.parseDouble(x_and_y[0]), Double.parseDouble(x_and_y[1]));
			}
			String [] x_and_y = id_and_coord[1].split("[XY]+");
			arena.setObstaclesCoords(Integer.parseInt(id_and_coord[0]), Double.parseDouble(x_and_y[0]), Double.parseDouble(x_and_y[1]));
		}
	}
	...
	public void setObstacleVcoords(String str) {
		...
		for (int i = 0; i < obstacles.length; i++) {
			...
			arena.setObstaclesVcoords(Integer.parseInt(id_and_vcoord[0]), Double.parseDouble(vcoord[0]), Double.parseDouble(vcoord[1]), Double.parseDouble(vcoord[2]), Double.parseDouble(vcoord[3]), Double.parseDouble(vcoord[4]));
		}
	}
}
